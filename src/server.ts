...
schedule.scheduleJob("00 20 * * *", async function () {
	...
	await Services.PushNotifications.send(
		...
		(user) => ({
			...
			body: `You've played ${user.dailyStreak} days in a row 🔥 Don't let your streak end today!`,
		})
	);
});
	...
	for (const key in mapUsersToAnswerCount) {
		...
		await Services.PushNotifications.send(
			...
			(user) => ({
				to: user.pushNotificationTokens,
				sound: "default",
				title: `Other players have found answers to ${answerCount} of your questions!`,
				body: `In the past 24 hours, the community has found answers to ${answerCount} questions asked by you. See the answers in the app.`,
			})
		);
	}
});
